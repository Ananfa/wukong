...
func genLobbyObject() {
	hFile, _ := os.Create(targetPath + pkgName + "_lobby_object.h")
	defer hFile.Close()

	genLobbyObjectHeadFile(hFile)

	cppFile, _ := os.Create(targetPath + pkgName + "_lobby_object.cpp")
	defer cppFile.Close()

	genLobbyObjectCppFile(cppFile)
}

func genLobbyObjectHeadFile(dstFile *os.File) {
	dstFile.WriteString("// This file is generated. Don't edit it\n" +
		"\n" +
		"#ifndef " + pkgName + "_lobby_object_h\n" +
		"#define " + pkgName + "_lobby_object_h\n\n" +
		...
		"\n" +
		"namespace " + pkgName +  " {\n" +
		"    class " + pkgNameCapFirst + "LobbyObject: public wukong::LobbyObject {\n" +
		"    public:\n" +
		"        " + pkgNameCapFirst + "LobbyObject(UserId userId, RoleId roleId, ServerId serverId, const std::string &lToken);\n" +
		"        virtual ~" + pkgNameCapFirst + "LobbyObject();\n" +
		"\n" +
	...
	dstFile.WriteString("    };\n" +
		"}\n" +
		"\n" +
		"#endif /* " + pkgName + "_lobby_object_h */\n")
}

func genLobbyObjectCppFile(dstFile *os.File) {
	dstFile.WriteString("// This file is generated. Don't edit it\n" +
		"\n" +
		"#include \"" + pkgName + "_lobby_object.h\"\n" +
		...
		"\n" +
		pkgNameCapFirst + "LobbyObject::" + pkgNameCapFirst + "LobbyObject(UserId userId, RoleId roleId, ServerId serverId, const std::string &lToken): wukong::LobbyObject(userId, roleId, serverId, lToken) {\n")
	...
	dstFile.WriteString("}\n" +
		"\n" +
		pkgNameCapFirst + "LobbyObject::~" + pkgNameCapFirst + "LobbyObject() {\n")
	...
	dstFile.WriteString("\n" +
		"bool " + pkgNameCapFirst + "LobbyObject::initData(const std::string &data) {\n" +
		"    auto msg = new wukong::pb::DataFragments;\n" +
		"    if (!msg->ParseFromString(data)) {\n" +
		"        ERROR_LOG(\"" + pkgNameCapFirst + "LobbyObject::initData -- parse role:%d data failed\\n\", roleId_);\n" +
		"        delete msg;\n" +
		"        return false;\n" +
		"    }\n" +
		"\n" +
	...
	fun1 := func(memberName, memberType string) {
		dstFile.WriteString("            auto msg1 = new " + memberType + ";\n" +
			...
			"                ERROR_LOG(\"" + pkgNameCapFirst + "LobbyObject::initData -- parse role:%d data--" + memberName + " failed\\n\", roleId_);\n" +
			"                delete msg1;\n" +
			"                delete msg;\n" +
			"                return false;\n" +
			"            }\n" +
	...
	dstFile.WriteString("void " + pkgNameCapFirst + "LobbyObject::buildSyncDatas(std::list<std::pair<std::string, std::string>> &datas, std::list<std::string> &removes) {\n" +
	...
	dstFile.WriteString("void " + pkgNameCapFirst + "LobbyObject::buildAllDatas(std::list<std::pair<std::string, std::string>> &datas) {\n")
	...
	dstFile.WriteString("void " + pkgNameCapFirst + "LobbyObject::onEnterGame() {\n" +
	...
	fun10 := func(memberName, memberType string) {
		...
		dstFile.WriteString(memberType + " " + pkgNameCapFirst + "LobbyObject::get" + mNameCapFirst + "() {\n" +
			"    return " + memberName + "_;\n" +
			"}\n" +
			"\n" +
			"void " + pkgNameCapFirst + "LobbyObject::set" + mNameCapFirst + "(" + memberType + " " + memberName + ") {\n" +
	...
	fun11 := func(memberName, memberType string) {
		...
		dstFile.WriteString(memberType + "* " + pkgNameCapFirst + "LobbyObject::get" + mNameCapFirst + "() {\n" +
			...
			"}\n" +
			"\n" +
			"void " + pkgNameCapFirst + "LobbyObject::set" + mNameCapFirst + "Dirty() {\n" +
			"    dirty_map_[\"" + memberName + "\"] = true;\n" +
			"}\n" +
			"\n")
	}
	...
	fun12 := func(memberName, memberType, itemType, itemId string) {
		mNameCapFirst := strings.Title(memberName)
		dstFile.WriteString("std::vector<uint32_t> " + pkgNameCapFirst + "LobbyObject::getAll" + mNameCapFirst + "Keys() {\n" +
			...
			"}\n" +
			"\n" +
			"bool " + pkgNameCapFirst + "LobbyObject::has" + mNameCapFirst + "(uint32_t " + itemId + ") {\n" +
			"    return " + memberName + "_map_.count(" + itemId + ") > 0;\n" +
			"}\n" +
			"\n" +
			"uint32_t " + pkgNameCapFirst + "LobbyObject::get" + mNameCapFirst + "Num() {\n" +
			"    return " + memberName + "_map_.size();\n" +
			"}\n" +
			"\n" +
			itemType + "* " + pkgNameCapFirst + "LobbyObject::get" + mNameCapFirst + "(uint32_t " + itemId + ") {\n" +
			...
			"}\n" +
			"\n" +
			"void " + pkgNameCapFirst + "LobbyObject::set" + mNameCapFirst + "Dirty(uint32_t " + itemId + ") {\n" +
			"    char dirtykey[50];\n" +
			"    sprintf(dirtykey,\"" + memberName + ".%d\"," + itemId + ");\n" +
			"    dirty_map_[dirtykey] = true;\n" +
			"}\n" +
			"\n" +
			"void " + pkgNameCapFirst + "LobbyObject::add" + mNameCapFirst + "(" + itemType + "* " + memberName + ") {\n" +
			"    " + memberName + "_map_.insert(std::make_pair(" + memberName + "->" + itemId + "(), " + memberName + "));\n" +
			"    set" + mNameCapFirst + "Dirty(" + memberName + "->" + itemId + "());\n" +
			"}\n" +
			"\n" +
			"void " + pkgNameCapFirst + "LobbyObject::remove" + mNameCapFirst + "(uint32_t " + itemId + ") {\n" +
			"    auto it = " + memberName + "_map_.find(" + itemId + ");\n" +
			"    if (it != " + memberName + "_map_.end()) {\n" +
			"        delete it->second;\n" +
