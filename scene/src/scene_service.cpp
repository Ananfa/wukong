...
void SceneServiceImpl::forwardIn(::google::protobuf::RpcController* controller,
                             const ::wukong::pb::ForwardInRequest* request,
                             ::corpc::Void* response,
                             ::google::protobuf::Closure* done) {

    auto role = g_SceneManager.getRole(request->roleid());
    if (!obj) {
        ERROR_LOG("SceneServiceImpl::forwardIn -- role[%llu] not found\n", request->roleid());
        return;
    }

    g_MessageHandleManager.handleMessage(obj, request->type(), request->tag(), request->rawmsg());

    //auto obj = g_LobbyObjectManager.getLobbyObject(request->roleid());
    //if (!obj) {
    //    ERROR_LOG("LobbyServiceImpl::forwardIn -- role[%llu] game object not found\n", request->roleid());
    //    return;
    //}
    //
    //g_MessageHandleManager.handleMessage(obj, request->type(), request->tag(), request->rawmsg());
...
//    stub->loadScene(controller, request, response, done);
    corpc::Controller *ctl = (corpc::Controller *)controller;

    // 判断manager中场景是否已经存在
    if (!request->sceneid().empty() && g_SceneManager.existScene(request->sceneid())) {
        ctl->SetErrorCode(SCENE_ALREADY_EXIST);
        return;
    }
    
    std::string sceneId = g_SceneManager.loadScene(request->defid(), request->sceneid(), request->roleid(), request->teamid());
    if (sceneId.empty()) {
        ctl->SetErrorCode(LOAD_SCENE_FAILED);
        return;
    }
...
//    stub->enterScene(controller, request, NULL, done);
    ...
    auto scene = g_SceneManager.getScene(request->sceneid());

    if (!scene) {
        ERROR_LOG("SceneServiceImpl::enterScene -- scene[%s] not found\n", request->sceneid().c_str());
        ctl->SetErrorCode(SCENE_NOT_EXIST);
        return;
    }
...
void SceneServiceImpl::leaveScene(::google::protobuf::RpcController* controller,
