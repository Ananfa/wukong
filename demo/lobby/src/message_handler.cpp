...
void MessageHandler::EnterSceneHandle(std::shared_ptr<GameObject> obj, uint16_t tag, std::shared_ptr<google::protobuf::Message> msg) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (result == REDIS_FAIL) { // 场景不存在，加载场景
            // 负载均衡找一个场景服
            uint32_t stype = 1;
            if (!g_ClientCenter.randomSceneServer(stype, sceneServerId)) {
            ...
            // 通知Scene服加载场景对象
            std::string tmpId = g_SceneClient.loadScene(sceneServerId, defId, sceneId, 0, "");
            if (tmpId.empty()) {
    ...
    // 进入场景
    g_SceneClient.enterScene(sceneServerId, sceneId, realObj->getRoleId(), realObj->getGatewayServerId());
}