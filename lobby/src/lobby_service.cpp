...
void LobbyServiceImpl::forwardIn(::google::protobuf::RpcController* controller,
...
void LobbyServiceImpl::enterGame(::google::protobuf::RpcController* controller,
    ...
    auto obj = g_LobbyObjectManager.getLobbyObject(request->roleid());
    if (!obj) {
        ERROR_LOG("LobbyServiceImpl::enterGame -- role[%llu] game object not found\n", request->roleid());
        return;
    }

    if (obj->getLToken() != request->ltoken()) {
        ERROR_LOG("LobbyServiceImpl::enterGame -- role[%llu] ltoken not match\n", request->roleid());
        return;
    }

    // 刷新gateway stub
    obj->setGatewayServerId(request->gatewayid());
